...
+ 部分场景下服务器开放了禁止下载允许上传并允许连接外网；使用命令将文件送出去

```shell
# 可连接的外网服务器开启监听
nc -l -p 18888 > received_file.tar.gz
# 当前内网服务器压缩并推送文件
tar -czf - /path/to/folder/ | nc 1.2.3.4 18888
```
...
-k # 重复接受并处理某个端口上所有连接，必须与-l选项一起使用
...
-X # 当nc客户端和代理服务器通信时，该选项指定它们之间使用的通信协议。nc默认使用的代理协议是SOCKS v.5
-x # 指定目标代理服务器的IP地址和端口号
nc -x remoteServer:port -X connect www.baidu.com port
-z # 扫描目标机上的某个或某些服务是否开启（端口扫描）
扫描某个主机在端口1-10000之间的服务，可以使用下面的命令：
lsof -z somehostname [1-10000]
```